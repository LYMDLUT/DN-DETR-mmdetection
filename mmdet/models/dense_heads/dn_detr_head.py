...
class DNDETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is DNDETRHead):
...
#                 'exactly the same.'
        ...
        self._init_layers()
        ###############################################
        self.label_enc = nn.Embedding(num_classes + 1, self.transformer.d_model - 1)
        ################################################
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is DNDETRHead:
            convert_dict = {
    ...
    def forward(self, feats, img_metas, gt_bboxes=None, gt_labels=None):
        """Forward function.
        ...
        if gt_bboxes is not None:
            gt_bboxes_list = [gt_bboxes for _ in range(num_levels)]
            gt_labels_list = [gt_labels for _ in range(num_levels)]
            img_metas_list = [img_metas for _ in range(num_levels)]
            return multi_apply(self.forward_single, feats, img_metas_list, gt_bboxes_list, gt_labels_list)
        else:
            img_metas_list = [img_metas for _ in range(num_levels)]
            return multi_apply(self.forward_single, feats, img_metas_list)


    def forward_single(self, x, img_metas, gt_bboxes=None, gt_labels=None):
...
#######################################################################
        # dn post process
        outputs_class, outputs_coord = dn_post_process(all_cls_scores, all_bbox_preds , mask_dict)
#######################################################################
...
##################################################################################################
    ...
    def forward_train(self,
        ...
        outputs_class, outputs_coord, mask_dict = self(x, img_metas, gt_bboxes, gt_labels)
        outs = (outputs_class, outputs_coord)
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas, mask_dict)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, mask_dict)
